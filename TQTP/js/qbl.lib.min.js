...
/**
 * 获取数据， url：文件地址， callback：成功调用方法，
 * variableName：json数据变量名称，默认tmp_www_szmb_gov_cn；
 */
var getJSON = function(url, callback, variableName) {
...
var createScript = function(src,callback,tag,arg){
...
function strToDate(str) {
	...
	var date = new Date(year, month, day, hour, minute, s);
	return date;
}
 ...
 * 
 * @param format
 *            [必需] 要输出的格式类型如 "yyyy-MM-dd hh:mm:ss"
 */
...
function _w_table_rowspan(_w_table_id, _w_table_colnum) {
	$(_w_table_id).each(function() {
		_w_table_firsttd = "";
		_w_table_currenttd = "";
		_w_table_SpanNum = 0;
		_w_table_Obj = $("tr td:nth-child(" + _w_table_colnum + ")", this);
	...
	$(_w_table_id)
			.each(
					function() {
						...
						_w_table_currenttd = "";
						_w_table_SpanNum = 0;
						$(" tr:nth-child(" + _w_table_rownum + ")", this)
								.each(
										function(i) {
...
(function() {
	 ...
	 *            String [必需] 文件路径
	 * @param type
	 *            String [必需] 数据类型：json , text
	 * @param name
	 *            Function [必需] 要查询的参数名称
	...
	/**
	 * 根据iframe的内容设置iframe的高度 该函数的常规调用方式为绑定iframe元素的onload事件，
		...
		if (document.getElementById) {
			if (cwin && !window.opera) {
				if (cwin.contentDocument
					...
					cwin.height = cwin.contentDocument.body.offsetHeight + 10; // FF
																				// NS
...
szmb.getWindLevel = function(l, renum) {
		if (typeof l == 'undefined')
			return;
		if (l <= 0.3)
			return renum == true ? 0 : '小于3级';
		else if (l <= 1.6)
			return renum == true ? 1 : '小于3级';
		else if (l <= 3.4)
			return renum == true ? 2 : '小于3级';
		else if (l <= 5.4)
			return renum == true ? 3 : '3级';
		else if (l <= 7.9)
			return renum == true ? 4 : '4级';
		else if (l <= 10.7)
			return renum == true ? 5 : '5级';
		else if (l <= 13.8)
			return renum == true ? 6 : '6级';
		else if (l <= 17.1)
			return renum == true ? 7 : '7级';
		else if (l <= 20.7)
			return renum == true ? 8 : '8级';
		else if (l <= 24.4)
			return renum == true ? 9 : '9级';
		else if (l <= 28.4)
			return renum == true ? 10 : '10级';
		else if (l <= 32.6)
			return renum == true ? 11 : '11级';
		else if (l > 32.6)
			return renum == true ? 12 : '12级';
	}
	...
	szmb.initMyRegionForTop = function() {
		var userfenqu;
		if (typeof MESIS_USER_AREA != 'undefined') {
			userfenqu = MESIS_USER_AREA;
		}
		if (userfenqu == null || userfenqu == ""
				|| typeof userfenqu == 'undefined') {
			userfenqu = vanwins.net.cookies.get('fenquyubaouserlikeregion');
		} else if (document.cookie != "") {
			vanwins.net.cookies.set('fenquyubaouserlikeregion', userfenqu,
					3600000);
		}
		if (userfenqu == null || userfenqu == ""
				|| typeof userfenqu == 'undefined') {
		...
		var temp = $("#MyRegionForTop option[value='" + userfenqu + "']").attr(
				"selected");
		if (typeof temp == 'undefined' || temp == false) {
			$("#MyRegionForTop option[value='" + userfenqu + "']").attr(
					"selected", true);
		}
	...
	szmb.SetMyRegionForTop = function(fenquPinyin, fenquZh) {
		...
		if (MESIS_USER_AREA_CN == null
				|| typeof MESIS_USER_AREA_CN == 'undefined') {
			var currentCookie = vanwins.net.cookies
					.get('fenquyubaouserlikeregion');
			vanwins.net.cookies.set('fenquyubaouserlikeregion', fenquPinyin,
					3600000);
			alert("设置成功! \n 您的默认分区为:" + fenquZh);
		} else {
...
/**
 * 加载一个js数据文件, 不建议直接使用该函数名称。 为与老系统兼容，声明此全局函数名称,该函数的功能由
 * vanwins.view.createScript(src,callback,tag,arg) 实现。 为避免命名冲突请使用
 * vanwins.view.createScript(src,callback,tag,arg) 调用
 * 
 * @param src
 *            String [必需] 文件路径
 * @param callback
...
/**
 * 不使用浏览器缓存，重新加载一个js数据文件 为与老系统兼容，声明此全局函数名称,该函数的功能由
 * vanwins.view.createScriptNoCache(src,callback,tag,arg) 实现。 为避免命名冲突请使用
 * vanwins.view.createScriptNoCache(src,callback,tag,arg) 调用
 * 
 * @param src
 *            String [必需] 文件路径
 * @param callback
...
/**
 * 根据iframe的内容设置iframe的高度 为与老系统兼容，声明此全局函数名称,该函数的功能由
 * vanwins.view.setCwinHeight(obj) 实现。 为避免命名冲突请使用
 * vanwins.view.setCwinHeight(obj) 调用 该函数的常规调用方式为绑定iframe元素的onload事件，
 * 当iframe页面内容包含异步加载时，该函数的常规调用方式并不能保证获取正确的高度。
 * 在iframe页面异加载操作完成的代码后面显式调用该函数能获取正确的高度。
 */
...
/**
 * 将String类型的日期(如："yyyy-MM-dd hh:mm:ss" )格式化为指定的中文日期字符串
 * 为与老系统兼容，声明此全局函数名称,该函数的功能由 vanwins.util.formatDateStr2CH(d,format,addDay,t)
 * 实现。 为避免命名冲突请使用 vanwins.util.formatDateStr2CH(d,format,addDay,t) 调用
 * 
 * @param d
 *            String [必需] 指定的日期
 * @param format
